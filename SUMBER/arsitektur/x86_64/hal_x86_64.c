/*
 * PIGURA OS - HAL x86_64
 * -----------------------
 * Implementasi Hardware Abstraction Layer untuk arsitektur x86_64.
 *
 * Berkas ini berisi implementasi fungsi-fungsi HAL yang spesifik
 * untuk arsitektur x86_64 (64-bit). Fungsi-fungsi ini mengabstraksi
...
/* State HAL global */
hal_state_t g_hal_state_x86_64;
...
static status_t _init_console_awal(void)
{
    ...
    /* VGA buffer di alamat fisik 0xB8000 */
    /* Di x86_64, kita perlu mapping ke higher half atau gunakan */
    /* identity mapping awal */
    vga = (volatile tak_bertanda16_t *)0xB8000;
 ...
 * hal_init
 * --------
 * Menginisialisasi HAL untuk arsitektur x86_64.
 */
status_t hal_init(void)
{
    kernel_memset(&g_hal_state_x86_64, 0, sizeof(hal_state_t));

    /* Inisialisasi console awal untuk debug */
    _init_console_awal();

    kernel_printf("[HAL-x86_64] Memulai inisialisasi HAL...\n");

    /* Remap PIC */
    kernel_printf("[HAL-x86_64] Remap PIC...\n");
    _remap_pic();

    /* Inisialisasi timer */
    kernel_printf("[HAL-x86_64] Inisialisasi timer...\n");
    g_hal_state_x86_64.timer.frequency = FREKUENSI_TIMER;
    g_hal_state_x86_64.timer.resolution_ns =
        (1000000000UL / FREKUENSI_TIMER);
    g_hal_state_x86_64.timer.supports_one_shot = SALAH;
    g_hal_state_x86_64.timer.supports_periodic = BENAR;
...
status_t hal_shutdown(void)
{
    ...
    kernel_printf("[HAL-x86_64] Mematikan HAL...\n");
...
{
    return g_hal_state_x86_64.timer.frequency;
}
...
{
    g_hal_state_x86_64.interrupt.irq_count = JUMLAH_IRQ;
    g_hal_state_x86_64.interrupt.vector_base = VEKTOR_IRQ_MULAI;
    g_hal_state_x86_64.interrupt.has_apic = SALAH;
    g_hal_state_x86_64.interrupt.has_ioapic = SALAH;
...
status_t hal_memory_init(void *bootinfo)
{
    ...
    if (bootinfo != NULL) {
        ...
        if (mbi->flags & MULTIBOOT_FLAG_MEM) {
            ...
            g_hal_state_x86_64.memory.total_bytes = total_mem;
            g_hal_state_x86_64.memory.available_bytes = total_mem;
        }
    }

    g_hal_state_x86_64.memory.page_size = UKURAN_HALAMAN;
    g_hal_state_x86_64.memory.page_count =
        g_hal_state_x86_64.memory.total_bytes / UKURAN_HALAMAN;

    /* x86_64 selalu mendukung PAE dan NX */
    g_hal_state_x86_64.memory.has_pae = BENAR;
    g_hal_state_x86_64.memory.has_nx = BENAR;
...
status_t hal_memory_get_info(hal_memory_info_t *info)
{
    ...
    kernel_memcpy(info, &g_hal_state_x86_64.memory,
...
{
    return g_hal_state_x86_64.memory.page_count;
}
...
status_t hal_console_init(void)
{
    ...
    /* VGA buffer (gunakan identity mapping awal) */
    vga = (volatile tak_bertanda16_t *)0xB8000;
