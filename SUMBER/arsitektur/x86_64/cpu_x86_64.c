/*
 * PIGURA OS - CPU x86_64
 * -----------------------
 * Implementasi fungsi-fungsi CPU untuk arsitektur x86_64.
 *
...
#define CPUID_LEAF_ADDR_WIDTH   0x80000008
...
/* Lebar alamat virtual dan fisik */
static tak_bertanda32_t g_alamat_virtual_lebar = 48;
static tak_bertanda32_t g_alamat_fisik_lebar = 40;
 ...
 */
static bool_t _deteksi_cpuid(void)
{
    tak_bertanda64_t flags1;
    tak_bertanda64_t flags2;
...
static void _baca_fitur(void)
{
    ...
    if (g_cpuid_max_ext >= 0x80000001) {
        ...
        if (ext_edx & CPUID_EXT_EDX_LM) {
            g_info_cpu.features |= (1 << 28); /* Long Mode */
        }
    }
}
...
static void _baca_cache(void)
{
    ...
    if (g_cpuid_max_ext >= 0x80000006) {
        ...
        g_info_cpu.cache_l2 = ((ecx >> 16) & 0xFFFF);
    }
}
...
/*
 * _deteksi_lebar_alamat
 * ---------------------
 * Mendeteksi lebar alamat virtual dan fisik.
 */
static void _deteksi_lebar_alamat(void)
{
    tak_bertanda32_t eax;
    tak_bertanda32_t ebx;
    tak_bertanda32_t ecx;
    ...
    /* Default untuk x86_64 */
    g_alamat_virtual_lebar = 48;
    g_alamat_fisik_lebar = 40;

    /* Baca dari extended leaf 0x80000008 */
    if (g_cpuid_max_ext >= 0x80000008) {
        _cpuid(0x80000008, 0, &eax, &ebx, &ecx, &edx);

        g_alamat_fisik_lebar = eax & 0xFF;
        g_alamat_virtual_lebar = (eax >> 8) & 0xFF;
    }
}
...
status_t hal_cpu_init(void)
{
    ...
    g_cpuid_tersedia = _deteksi_cpuid();
    ...
    kernel_printf("[CPU-x86_64] Vendor: %s\n", g_info_cpu.vendor);
    kernel_printf("[CPU-x86_64] Brand: %s\n", g_info_cpu.brand);
    kernel_printf("[CPU-x86_64] Family: %u, Model: %u, Stepping: %u\n",
 ...
 * cpu_write_cr0
 * -------------
 * Menulis ke register CR0.
 *
 * Parameter:
 *   val - Nilai yang akan ditulis
 */
void cpu_write_cr0(tak_bertanda64_t val)
{
    __asm__ __volatile__(
        "mov %0, %%cr0"
        :
 ...
 * cpu_read_cr2
 * ------------
 * Membaca register CR2.
 *
 * Return:
 *   Nilai CR2
 */
tak_bertanda64_t cpu_read_cr2(void)
{
    tak_bertanda64_t val;
    __asm__ __volatile__(
        "mov %%cr2, %0"
 ...
 * cpu_read_cr3
 * ------------
 * Membaca register CR3.
 *
 * Return:
 *   Nilai CR3
 */
tak_bertanda64_t cpu_read_cr3(void)
{
    tak_bertanda64_t val;
    __asm__ __volatile__(
        "mov %%cr3, %0"
 ...
 * cpu_write_cr3
 * -------------
 * Menulis ke register CR3.
 *
 * Parameter:
 *   val - Nilai yang akan ditulis
 */
void cpu_write_cr3(tak_bertanda64_t val)
{
    __asm__ __volatile__(
        "mov %0, %%cr3"
        :
 ...
 *   Nilai CR4
 */
tak_bertanda64_t cpu_read_cr4(void)
{
    tak_bertanda64_t val;
    __asm__ __volatile__(
        "mov %%cr4, %0"
 ...
 *   val - Nilai yang akan ditulis
 */
void cpu_write_cr4(tak_bertanda64_t val)
{
    __asm__ __volatile__(
        "mov %0, %%cr4"
        :
 ...
 *   Nilai MSR
 */
tak_bertanda64_t cpu_read_msr(tak_bertanda32_t msr)
{
    return _baca_msr(msr);
}
 ...
 *   val  - Nilai yang akan ditulis
 */
void cpu_write_msr(tak_bertanda32_t msr, tak_bertanda64_t val)
{
    _tulis_msr(msr, val);
}

/*
 * cpu_get_phys_addr_width
 * -----------------------
 * Mendapatkan lebar alamat fisik.
 *
 * Return:
 *   Lebar alamat fisik dalam bit
 */
tak_bertanda32_t cpu_get_phys_addr_width(void)
{
    return g_alamat_fisik_lebar;
}

/*
 * cpu_get_virt_addr_width
 * -----------------------
 * Mendapatkan lebar alamat virtual.
 *
 * Return:
 *   Lebar alamat virtual dalam bit
 */
tak_bertanda32_t cpu_get_virt_addr_width(void)
{
    return g_alamat_virtual_lebar;
}

/*
 * cpu_enable_syscall
 * ------------------
 * Mengaktifkan instruksi SYSCALL/SYSRET.
 */
void cpu_enable_syscall(void)
{
    tak_bertanda64_t efer;
    tak_bertanda64_t star;

    /* Baca EFER */
    efer = _baca_msr(MSR_EFER);

    /* Set SCE bit */
    efer |= EFER_SCE;

    /* Tulis kembali EFER */
    _tulis_msr(MSR_EFER, efer);

    /* Setup STAR MSR */
    /* Bits 32-47: Kernel CS (also SS = CS + 8) */
    /* Bits 48-63: User CS (also SS = CS + 8) */
    star = ((tak_bertanda64_t)(SELECTOR_KERNEL_CODE) << 32) |
           ((tak_bertanda64_t)(SELECTOR_USER_CODE | 0x03) << 48);
    _tulis_msr(MSR_STAR, star);

    /* Setup LSTAR (syscall entry point) */
    extern void _syscall_entry(void);
    _tulis_msr(MSR_LSTAR, (tak_bertanda64_t)_syscall_entry);

    /* Setup SF_MASK (flags to clear on syscall) */
    _tulis_msr(MSR_SF_MASK, 0x3F7FD5);
}
