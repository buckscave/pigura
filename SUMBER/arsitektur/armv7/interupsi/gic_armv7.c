/*
 * PIGURA OS - GIC_ARMV7.C
 * ------------------------
 * Implementasi Generic Interrupt Controller untuk ARMv7.
 *
 * Berkas ini berisi fungsi-fungsi untuk mengelola GICv2
 * (Generic Interrupt Controller version 2) pada ARMv7.
 *
...
/* GIC Virtual Interface base address */
#define GICV_BASE               0x08020000

/* GIC Virtual CPU Interface base address */
...
#define GICD_NSACR              0xE00       /* Non-secure Access Control */
...
#define GICC_AIAR               0x020       /* Aliased Interrupt Ack */
#define GICC_AEOIR              0x024       /* Aliased End of Interrupt */
#define GICC_AHPPIR             0x028       /* Aliased Highest Priority */
...
#define GICC_CTLR_EOImodeNS     (1 << 9)    /* EOImode for NS */
...
status_t hal_gic_set_target(tak_bertanda32_t irq, tak_bertanda8_t target)
{
    ...
    /* Hanya untuk SPI (Shared Peripheral Interrupts) */
 ...
 * hal_gic_set_config
 * ------------------
 * Set konfigurasi IRQ (edge/level triggered).
 *
 * Parameter:
 *   irq    - Nomor IRQ
 *   config - Konfigurasi (0 = level, 1 = edge)
 *
 * Return: Status operasi
 */
status_t hal_gic_set_config(tak_bertanda32_t irq, tak_bertanda32_t config)
{
    tak_bertanda32_t offset;
    tak_bertanda32_t shift;
    tak_bertanda32_t reg;
    ...
    tak_bertanda32_t filter)
{
    ...
    /* Build SGIR value */
    sgir = ((target & 0xFF) << 16) | ((filter & 0x3) << 24) |
           (sgi_id & 0xF);
...
status_t hal_gic_init(void)
{
    ...
    /* Set semua interrupt ke Group 0 (secure) */
    for (i = 0; i < g_gic_irq_count; i += 32) {
