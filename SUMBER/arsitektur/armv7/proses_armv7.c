/*
 * PIGURA OS - PROSES_ARMV7.C
 * ---------------------------
 * Implementasi manajemen proses untuk arsitektur ARMv7.
 *
 * Berkas ini berisi fungsi-fungsi untuk mengelola proses dan thread
 * pada prosesor ARMv7, termasuk context switching dan scheduler.
 *
...
#define UKURAN_STACK_THREAD     (8UL * 1024UL)   /* 8 KB */
#define UKURAN_STACK_KERNEL     (16UL * 1024UL)  /* 16 KB */
...
typedef struct {
    ...
    tak_bertanda32_t sp;        /* r13 */
    tak_bertanda32_t lr;        /* r14 */
    tak_bertanda32_t pc;        /* r15 */

    /* Status register */
    tak_bertanda32_t cpsr;
    tak_bertanda32_t spsr;

    /* FPU/VFP registers (if used) */
    tak_bertanda32_t fpu_used;
    tak_bertanda64_t vfp[32];   /* D0-D31 */
    tak_bertanda32_t fpscr;
} cpu_context_t;
...
static proses_pcb_t *proses_cari_dengan_pid(pid_t pid)
{
    ...
    for (i = 0; i < MAKS_PROSES_INTERNAL; i++) {
        if (g_process_table[i].digunakan && g_process_table[i].pid == pid) {
 ...
 * Fungsi ini dipanggil dari assembly.
 *
 * Parameter:
...
status_t hal_proses_context_switch(proses_pcb_t *proses_baru)
{
    ...
    /* Restore FPU context jika diperlukan */
    if (proses_baru->fpu_used) {
        hal_proses_fpu_restore(&proses_baru->context);
    }
    ...
    bool_t is_kernel)
{
    ...
    proses->tid = (tid_t)proses->pid;  /* TID = PID untuk main thread */
    ...
    if (is_kernel) {
        ...
        /* User stack akan dialokasikan oleh memory manager */
        proses->stack_top = 0;
    }
    ...
    proses->context.lr = 0;  /* Return address */

    /* Set CPSR */
    if (is_kernel) {
        /* Kernel mode: SVC mode dengan IRQ enabled */
 ...
 */
void hal_proses_fpu_save(cpu_context_t *context)
{
    tak_bertanda32_t i;
    ...
    /* Save VFP registers D0-D31 */
    for (i = 0; i < 32; i++) {
        __asm__ __volatile__(
            "vmov %0, %1, d%2"
            : "=r"(((tak_bertanda32_t *)&context->vfp[i])[0]),
              "=r"(((tak_bertanda32_t *)&context->vfp[i])[1])
            : "i"(i)
        );
    }
 ...
 *   context - Pointer ke context
 */
void hal_proses_fpu_restore(cpu_context_t *context)
{
    tak_bertanda32_t i;
 ...
 * --------------------
 * Inisialisasi subsistem proses arsitektur ARMv7.
 *
 * Return: Status operasi
 */
