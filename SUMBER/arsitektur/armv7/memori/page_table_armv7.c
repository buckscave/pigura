/*
 * PIGURA OS - PAGE_TABLE_ARMV7.C
 * -------------------------------
 * Implementasi manajemen page table untuk ARMv7.
 *
 * Berkas ini berisi fungsi-fungsi untuk mengelola page table
 * dengan format short descriptor (32-bit).
 *
...
#define PAGE_XN             (1 << 4)    /* Execute Never */
...
#define PAGE_APX            (1 << 15)   /* Access Permission extend */
#define PAGE_S              (1 << 16)   /* Shareable */
#define PAGE_NG             (1 << 17)   /* Not Global */
#define PAGE_NS             (1 << 19)   /* Non-secure */

/* Access permissions */
#define AP_NO_ACCESS        0x00    /* No access */
#define AP_PRIV_RW          0x01    /* R/W privileged */
#define AP_PRIV_RO          0x02    /* R/O privileged */
#define AP_FULL_RW          0x03    /* R/W all modes */

/* Page sizes */
#define SECTION_SIZE        (1024 * 1024)  /* 1 MB */
#define LARGE_PAGE_SIZE     (64 * 1024)    /* 64 KB */
#define SMALL_PAGE_SIZE     (4 * 1024)     /* 4 KB */
#define TINY_PAGE_SIZE      (1 * 1024)     /* 1 KB */

/* Number of entries per level */
#define L1_ENTRIES          4096
#define L2_ENTRIES_COARSE   256
#define L2_ENTRIES_FINE     1024
    ...
    tak_bertanda32_t flags)
{
    ...
    /* Large page harus 64 KB aligned, dan index harus kelipatan 16 */
