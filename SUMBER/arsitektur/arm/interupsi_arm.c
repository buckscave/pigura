/*
 * PIGURA OS - INTERUPSI_ARM.C
 * ----------------------------
 * Implementasi interface interupsi untuk arsitektur ARM (32-bit).
 *
 * Berkas ini berisi fungsi-fungsi untuk mengelola interupsi pada
 * prosesor ARM, termasuk setup vector table dan dispatch handler.
 *
...
#define VEC_UNDEFINED       0x04
#define VEC_SWI             0x08
#define VEC_PREFETCH_ABORT  0x0C
#define VEC_DATA_ABORT      0x10
#define VEC_UNUSED          0x14
#define VEC_IRQ             0x18
#define VEC_FIQ             0x1C
...
/* IRQ counter */
static tak_bertanda64_t g_irq_count[MAX_IRQ_ENTRIES];
...
void hal_dispatch_irq(void)
{
    ...
    /* Baca Interrupt Acknowledge Register dari GIC/IRQ controller */
    ...
    /* Acknowledge IRQ di interrupt controller */
 ...
 * hal_dispatch_fiq
 * ----------------
 * Dispatch FIQ ke handler yang sesuai.
 */
void hal_dispatch_fiq(void)
{
 ...
 * -----------------------
 * Inisialisasi subsistem interupsi arsitektur ARM.
 *
 * Return: Status operasi
 */
