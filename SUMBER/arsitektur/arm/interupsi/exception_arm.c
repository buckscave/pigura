/*
 * PIGURA OS - EXCEPTION_ARM.C
 * ----------------------------
 * Implementasi handler exception untuk ARM (32-bit).
 *
...
#define FSR_TRANSLATION_L1      0x05
#define FSR_TRANSLATION_L2      0x07
#define FSR_ACCESS_L1           0x03
#define FSR_ACCESS_L2           0x06
#define FSR_DOMAIN_L1           0x09
#define FSR_DOMAIN_L2           0x0B
#define FSR_PERMISSION_L1       0x0D
#define FSR_PERMISSION_L2       0x0F
...
/*
 * exception_swi_handler
 * ---------------------
 * Handler untuk SWI (system call) exception.
 *
 * Parameter:
 *   frame - Stack frame exception
 */
void exception_swi_handler(exception_frame_t *frame)
{
    tak_bertanda32_t syscall_num;
    tak_bertanda64_t result;

    g_exception_count[EXC_SWI]++;
...
void exception_data_abort_handler(exception_frame_t *frame)
{
    ...
    if ((frame->spsr & CPSR_MODE_MASK) == CPSR_MODE_USR) {
        switch (fault_type) {
            case FSR_TRANSLATION_L1:
            case FSR_TRANSLATION_L2:
                hal_proses_exit(SIGNAL_SEGFAULT);
                break;

            case FSR_PERMISSION_L1:
            case FSR_PERMISSION_L2:
                hal_proses_exit(SIGNAL_SEGFAULT);
                break;
...
status_t hal_exception_init(void)
{
    ...
    hal_exception_register_handler(EXC_UNDEFINED,
        exception_undefined_handler);
    hal_exception_register_handler(EXC_SWI,
        exception_swi_handler);
