...
/* Flag wait - lokal untuk penggunaan internal */
...
/* Wait state untuk internal tracking */
#define WAIT_STATE_NONE         0
#define WAIT_STATE_WAITING      1

/* P_* constants untuk waitid */
#define P_ALL                   0
#define P_PID                   1
#define P_PGID                  2
...
static struct {
    ...
    tak_bertanda64_t nohang_returns;
} wait_stats = {0, 0, 0, 0, 0};
                     ...
                     void *infop, tak_bertanda32_t options)
{
    ...
    if (infop != NULL) {
        ...
        if (child != NULL) {
            siginfo_t *info = (siginfo_t *)infop;
            info->si_signo = SIGCHLD;
            info->si_pid = child->pid;
            info->si_uid = child->uid;
            info->si_status = status >> WAIT_STATUS_SHIFT;
...
void wait_print_stats(void)
{
    ...
    kernel_printf("  NoHang Returns : %lu\n", wait_stats.nohang_returns);
    kernel_printf("========================================\n");
}
